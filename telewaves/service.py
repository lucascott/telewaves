...
class TeleWaves:
    """
    Main service class for monitoring Telegram messages and downloading audio files.

    This service acts as your personal Telegram user account to monitor all incoming
    messages for audio files, automatically downloading them to a specified directory.
    Supports chat filtering to monitor only specific users or channels.
    ...
    Attributes:
        ...
        audio_extensions (set[str]): Supported audio file extensions
    ...
    def __init__(
        ...
        Args:
            ...
            download_dir (Path): Directory to save downloaded audio files
    ...
    def is_audio_file(self, file_path: Path) -> bool:
        """
        Check if a file is an audio file based on its file extension.
    ...
    async def process_message(self, event: events.NewMessage.Event):
        """
        Process incoming Telegram messages to detect and download audio files.
        ...
        if isinstance(media, MessageMediaDocument):
            ...
            if document.mime_type and (
                ...
                or document.mime_type in ["video/mp4", "video/x-msvideo"]
            ):
                should_download = True
                logger.info(
                    f"Found audio document: {filename} (MIME: {document.mime_type})"
                )
            elif filename and Path(filename).suffix.lower() in self.audio_extensions:
                should_download = True
                logger.info(f"Found audio file by extension: {filename}")
            else:
                logger.debug(
                    f"Document {filename} is not an audio file (MIME: {document.mime_type})"
                )

        if should_download and filename:
            try:
                # Download the file
                ...
                if downloaded_path and downloaded_path.exists():
                    ...
                    if self.is_audio_file(downloaded_path):
                        logger.info(
                            f"Successfully processed audio file: {downloaded_path}"
                        )
                        ...
                        logger.warning(
                            f"Downloaded file is not an audio file: {downloaded_path}"
                        )
                        # Optionally remove non-audio files
                        downloaded_path.unlink(missing_ok=True)
                        logger.info(f"Removed non-audio file: {downloaded_path}")
                else:
                    logger.error(
    ...
    async def run(self):
        ...
        manually stopped or disconnected.
        ...
        try:
            ...
            logger.info("Monitoring your Telegram messages for audio files...")
